...
class VendorPerformanceAPITest(TestCase):
    def setUp(self):
        ...
        # URL for vendor performance API (adjust if needed)
        self.vendor_performance_url = reverse('vendor-performance', kwargs={'vendor_id': self.vendor.pk})
    ...
    def test_vendor_performance_success(self):
        ...
        self.assertEqual(response.json()[0]['on_time_delivery_rate'], self.performance_record.on_time_delivery_rate)
        
    def test_vendor_performance_not_found(self):
        invalid_vendor_url = reverse('vendor-performance', kwargs={'vendor_id': 9999})  # A non-existent vendor ID
